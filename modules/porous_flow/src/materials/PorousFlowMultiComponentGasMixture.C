    ...
    else
    {
      ...
      for (unsigned int i = 0; i < _n_components; i++)
      {
        ...
        if (i == _n_components - 1)
        {
          ...
          mu_sum += (mu * (1 - total_mass_frac)) / _pressure_to_Pascals / _time_to_seconds;
          drho_dp_sum +=
              (drho_dp * MetaPhysicL::raw_value((1 - total_mass_frac))) * _pressure_to_Pascals;
          drho_dT_sum += drho_dT * MetaPhysicL::raw_value((1 - total_mass_frac));
          dmu_dp_sum += (dmu_dp * MetaPhysicL::raw_value((1 - total_mass_frac))) / _time_to_seconds;
        ...
        else
        {
          ...
          (*_dviscosity_dX)[_qp][i] = mu - mu_end;
          rho_sum += rho * (*_x[i])[_qp];
          mu_sum += (mu * (*_x[i])[_qp]) / _pressure_to_Pascals / _time_to_seconds;
          drho_dp_sum += (drho_dp * MetaPhysicL::raw_value((*_x[i])[_qp])) * _pressure_to_Pascals;
          drho_dT_sum += drho_dT * MetaPhysicL::raw_value((*_x[i])[_qp]);
          dmu_dp_sum += (dmu_dp * MetaPhysicL::raw_value((*_x[i])[_qp])) / _time_to_seconds;
          dmu_dT_sum += (dmu_dT * MetaPhysicL::raw_value((*_x[i])[_qp])) / _pressure_to_Pascals /
                        _time_to_seconds;
          total_mass_frac += (*_x[i])[_qp];
        }
      }
      ...
      (*_viscosity)[_qp] = mu_sum;
      (*_ddensity_dp)[_qp] = drho_dp_sum;
      (*_ddensity_dT)[_qp] = drho_dT_sum;
      (*_dviscosity_dp)[_qp] = dmu_dp_sum;
      (*_dviscosity_dT)[_qp] = dmu_dT_sum;
    }
  }
}
